...
4. Record results: `<command> | exp run record "$RUN" --output -`
...
| `exp run record <run-id>` | Record JSON output for a run |
        ...
        "json" => {
            let guide = serde_json::json!({
                "workflow_steps": [
                    ...
                    {"order": 4, "command": "<command> | exp run record \"$RUN\" --output -", "purpose": "Pipe structured JSON output into the run"},
